...
module.exports = (function() {
  ...
  */
  ...
  var isUpdateQuery = function() {
    ...
    var handleSelectQuery = function (results) {
        ...
        if (results.length === 0) {
            result = [];
        } else if (this.options.pojos) {

            result = directToPojos(
                results,
                {
            ...
            result = results.map(function (result) {
                ...
                var joinTableData = result[this.options.joinTableModel.name]
                    , joinTableDAO = this.options.joinTableModel.build(joinTableData,
                                                                       {isNewRecord: false, isDirty: false})
                    , mainDao
  ...
  */
  ...
  var groupJoinData = function(data, includeOptions, options) {
    ...
    var findArrRev = function(collection, predicate) {
    ...
    var omitFast = function(source, omitKeys) {
        var target = {};
        var keys = Object.keys(source);
        var i = keys.length;
        while(i--) {
            var key = keys[i];
            if (omitKeys[key] === undefined) {
    ...
    // includeOptions are 'level'-specific where options is a general directive
    var directToPojos = function(data, includeOptions, options) {
        var results = []
            , resultsByPk = {}
            , i
            , includeMap = includeOptions.includeMap || {}
            , includeNames = includeOptions.includeNames
            , existingResult
            , calleeData
            , calleePrimaryKey
            , child
            , calleeDataIgnore = {'__children': true}
            , parseChildren = function(result) {
                _.each(result.__children, function (children, key) {

                    if (key.substr(0, 4) === 'Xref') {
                        return;
                    }

                    var include = includeMap[key] || {};
                    var association = include.association;

                    var childPojos = directToPojos(children, include, options);
                    ...
                    if (association) {
                        switch(association.associationType) {
                            ...
                            case 'HasOne':
                            case 'BelongsTo':
                                var id = association.identifier;
                                key = id.substr(0, id.length - 2);
                                childPojos = childPojos[0];
                                break;

                            case 'HasMany':
                                var as = association.as;
                                key = as.charAt(0).toLowerCase() + as.substr(1);
                                break;
                        }
                    }

                    result[key] = childPojos;
                });
                delete result.__children;
            },
            primaryKeyAttribute
        ...
        // Identify singular primaryKey attribute for equality check (if possible)
        var pka = includeOptions.daoFactory.primaryKeyAttributes;
        if (pka.length === 1) {
            primaryKeyAttribute = pka[0];
        } else if (includeOptions.daoFactory.rawAttributes.id) {
            primaryKeyAttribute = 'id';
        }
        ...
        // Ignore all include keys on main data
        if (includeNames) {
            i = includeNames.length;
            while(i--) {
                calleeDataIgnore[includeNames[i]] = true;
            }
        }
        ...
        data.forEach(function (row) {
            row = Dot.transform(row);
            calleeData = omitFast(row, calleeDataIgnore);
            calleePrimaryKey = calleeData[primaryKeyAttribute];

            // If there are :M associations included we need to see if the main result of the row has already been identified
            existingResult = options.checkExisting &&
                (primaryKeyAttribute
                        ? resultsByPk[calleePrimaryKey]
                        : findArrRev(results,
                                     function (result) {
                                         return Utils._.isEqual(omitFast(result, calleeDataIgnore), calleeData)
                                     })
                );
            ...
            if (!existingResult) {
                results.push(existingResult = calleeData)
                if (primaryKeyAttribute) {
                    resultsByPk[calleePrimaryKey] = existingResult;
                }
            }

            (rowKeys || (rowKeys = Object.keys(row))).forEach(
                function(attrName) {
                    // Child if object, and is an child include
                    child = Object(row[attrName]) === row[attrName] && includeMap[attrName];

                    if (child) {
                        // Make sure nested object is available
                        var c;
                        if ( (c = existingResult.__children) === undefined) {
                            existingResult.__children = c = {};
                        }
                        var a;
                        if ( ( a = c[attrName]) === undefined) {
                            c[attrName] = a = [];
                        }
